...
class Solver(object):
    ...
    def fit(self, train_data, eval_data=None,
        ...
        self.symbol.infer_shape(data=train_data.provide_data[0][1])
        ...
        self.aux_params = {k : mx.nd.zeros(s) \
        ...
        for epoch in range(self.begin_epoch, self.num_epoch):
            ...
            for data in train_data:
                ...
                self.arg_params[data_name] = \
                mx.nd.array(data[data_name], self.ctx)
                self.arg_params[label_name] = \
                mx.nd.array(data[label_name].reshape(label_shape[0], \
                                                     label_shape[1]*label_shape[2]), self.ctx)
                output_names = self.symbol.list_outputs()


                self.exector = self.symbol.bind(ctx=self.ctx, args=self.arg_params,
                                               args_grad=self.grad_params,
                                               grad_req=grad_req,
                                               aux_states=None,
                                               group2ctx=None,
                                               shared_exec=None)
                assert len(self.symbol.list_arguments()) == \
                len(self.exector.grad_arrays)
            ...
            if eval_data:
                ...
                for data in eval_data:
                    nbatch += 1
                    label_shape = data[label_name].shape
                    self.arg_params[data_name] = mx.nd.array(data[data_name], \
                                                             self.ctx)
